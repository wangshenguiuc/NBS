function [indClust,p2p] = aggregate_cluster_consensus( cluster_matrix )
...
for i=1:npat
    ...
    for j=1:npat
        ...
        sum_vec = cluster_matrix(i,:) + cluster_matrix(j,:); %both zero means not sampled
        p2p(i,j) = p2p(i,j) + length(find(diff_vec==0)) - length(find(sum_vec==0));
        %         for m=1:nsample
