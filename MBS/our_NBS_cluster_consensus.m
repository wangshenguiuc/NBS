function [clust_pv,indClust_all,chisq,cluster_max,cluster_min] = our_NBS_cluster_consensus(pat_mute,integrate_mat,pat_survival,rst_prob,selected_pat,i2p,degree,cancer_type,max_nclst,min_nclst,...
...
nconsensus = 10;
sample_ratio = 0.8;
npara = length(dim_l)*length(dist_type_l)*length(input_mat_type_l)*nconsensus;
nmat = length(integrate_mat);
dist_mat = cell(nmat,npara);
sample_pat_c = cell(nmat,npara);
        ...
        for input_mat_type = input_mat_type_l
            ...
            for dim = dim_l
                ...
                for low_d_type = low_d_type_l
                    ...
                    for nconsensusi=  1:nconsensus
                        sample_dim = randsample(dim_i,round(dim_i*sample_ratio),true);
                        sample_pat = randsample(npat,round(npat*sample_ratio),false);
                        low_d_mat_sample = low_d_mat(sample_pat,sample_dim);
                        for dist_type = dist_type_l
                            if dist_type == 1
                                dist = squareform(pdist(low_d_mat_sample,'cosine'));
                            elseif dist_type == 2
                                dist = squareform(pdist(low_d_mat_sample));
                            elseif dist_type == 3
                                dist = squareform(pdist(low_d_mat_sample,'cosine'));
                            ...
                            sample_pat_c{mati,ct} = sample_pat;
                            ct = ct + 1;
...
if nmat>1 || size(integrate_mat{1},2)~=1
    ...
    for pi = 1:npara
        ...
        for mati=1:nmat
            ...
            sample_pat = sample_pat_c{mati,pi};
            
            for clst_type = clst_type_l
                for nclst=min_nclst:max_nclst
                    if clst_type == 1
                        rng(1);
                        [indClust_tmp,~,sumD] = kmeans(full(dist),nclst,'replicates',100);
                    elseif clst_type == 2
                        rng(1);
                        [indClust_tmp,~,sumD] = kmeans(full(dist),nclst,'distance','cosine','replicates',100);
                    end
                    indClust = zeros(npat,1);
                    indClust(sample_pat) = indClust_tmp;
                    consen_indClust{nclst} = [consen_indClust{nclst},indClust];
                end
            end
...
for nclst=min_nclst:max_nclst
    ...
    indClust_prefix = [cancer_type,'_',num2str(nclst),'_',num2str(rst_prob),'_',num2str(diff_mat_type),'_',num2str(input_mat_type), ...
        '_',num2str(clst_type), ...
        '_',num2str(dim),'_',num2str(dist_type),method_name,'.txt'];
    
    
    indClust = aggregate_cluster_consensus(consen_indClust{nclst});
